...
async function run(): Promise<void> {
  try {
    ...
    await loginToCodex(inputs, workspace);
...
async function installCliTools(): Promise<void> {
  ...
  await exec.exec('npm', ['install', '-g', '@openai/codex@0.46.0']);
...
async function loginToCodex(inputs: Inputs, workspace: string): Promise<void> {
  const env = {
    ...process.env,
...
async function removeAutofixPlanIfCodeChanges(workspace: string): Promise<void> {
